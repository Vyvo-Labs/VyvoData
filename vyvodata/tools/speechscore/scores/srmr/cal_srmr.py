...
from vyvodata.tools.speechscore.scores.srmr.modulation_filters import (
...
from vyvodata.tools.speechscore.scores.srmr.segmentaxis import segment_axis
...
def calc_cutoffs(cfs, fs, q):
    ...
    w0 = 2 * np.pi * cfs / fs
    b0 = np.tan(w0 / 2) / q
    left = cfs - (b0 * fs / (2 * np.pi))
    right = cfs + (b0 * fs / (2 * np.pi))
    return left, right
...
def cal_srmr(
    ...
    wlength = int(np.ceil(wlength_s * mfs))
    winc = int(np.ceil(winc_s * mfs))
    ...
    k90_perc_idx = np.where(ac_perc_cumsum > 90)[0][0]

    bw = erbs[k90_perc_idx]
