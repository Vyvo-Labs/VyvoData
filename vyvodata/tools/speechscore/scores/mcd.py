...
class MCD(ScoreBasis):
    def __init__(self):
        ...
        self.mcd_toolbox = CalculateMcd(mcd_mode="plain")
...
class CalculateMcd:
    ...
    def __init__(self, mcd_mode):
        ...
        self.log_spec_dB_const = (
            10.0 / math.log(10.0) * math.sqrt(2.0)
        )  # 6.141851463713754
    ...
    def calculate_mcd_distance(self, x, y, path):
        ...
        """
        pathx = list(map(lambda item: item[0], path))
        pathy = list(map(lambda item: item[1], path))
        x, y = x[pathx], y[pathy]
    ...
    def average_mcd(
        self, loaded_ref_wav, loaded_syn_wav, cost_function, mcd_mode, score_rate
    ):
        """
        ...
        if mcd_mode == "plain":
            ...
            if len(loaded_ref_wav) < len(loaded_syn_wav):
                loaded_ref_wav = np.pad(
                    loaded_ref_wav, (0, len(loaded_syn_wav) - len(loaded_ref_wav))
                )
            else:
                loaded_syn_wav = np.pad(
                    loaded_syn_wav, (0, len(loaded_ref_wav) - len(loaded_syn_wav))
                )
            ...
            if mcd_mode == "plain":
                ...
                _, path = fastdtw(
                    ref_mcep_vec[:, 1:], syn_mcep_vec[:, 1:], dist=euclidean
                )
            ...
            frames_tot, min_cost_tot = self.calculate_mcd_distance(
                ref_mcep_vec, syn_mcep_vec, path
            )
