...
class NisqaDe(nn.Module):
    """
    NisqaDe: The main speech quality model for double-ended prediction.
    The module loads the submodules for framewise modelling (e.g. CNN),
...
class DFF(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch_f.relu(self.bn2(self.lin1(x)))
        x = self.dropout(x)
        x = torch_f.relu(self.bn3(self.lin2(x)))
        x = self.dropout(x)
        x = torch_f.relu(self.bn4(self.lin3(x)))
        x = self.dropout(x)
        x = torch_f.relu(self.bn5(self.lin4(x)))
...
class AdaptCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch_f.adaptive_max_pool2d(x, output_size=(self.pool_1))

        x = torch_f.relu(self.bn2(self.conv2(x)))
        x = torch_f.adaptive_max_pool2d(x, output_size=(self.pool_2))

        x = self.dropout(x)
        x = torch_f.relu(self.bn3(self.conv3(x)))
        x = self.dropout(x)
        x = torch_f.relu(self.bn4(self.conv4(x)))
        x = torch_f.adaptive_max_pool2d(x, output_size=(self.pool_3))

        x = self.dropout(x)
        x = torch_f.relu(self.bn5(self.conv5(x)))
        x = self.dropout(x)
        x = torch_f.relu(self.bn6(self.conv6(x)))
...
class Alignment(torch.nn.Module):
    ...
    def forward(self, query, y, n_wins_y):
        if self.att is not None:
            ...
            att_score = torch_f.softmax(att_score, dim=2)
